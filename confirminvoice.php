...
<div id="contentheaderspace"></div>
        <div id="content">
		<p class="warningfont">Transactions added today were found for this account, would you like to continue one of those?</p>
...
$sql1 = 'SELECT `id`,`invoicedate`,`type`,`total`,`vehicleid`,`mileagein`,`taxgroup` FROM `invoice` WHERE `accountid` = :accountid AND `invoicedate` LIKE :startdate ORDER BY `invoicedate` DESC';
$sth1 = $pdocxn->prepare($sql1);
$sth1->bindParam(':accountid',$accountid);
$sth1->bindParam(':startdate',$currentday2);
$sth1->execute();
	while($row1 = $sth1->fetch(PDO::FETCH_ASSOC))
	{
...
<td class=\"center\"><a href=\"invoice.php?invoiceid=".$id."\" class=\"no-decoration\"><input type=\"button\" class=\"smallbutton\" name=\"submit\" value=\"Use this ".$typename."\"></a></td></tr></table></div><div class=\"q3\"><table class=\"righttable\"><tr><th>qty</th><th>comment</th><th>unit cost</th><th>total cost</td></tr>\n";
}else{
...
echo "<tr href=\"$tri\"><td colspan=\"5\" class=\"center\"><a href=\"invoice.php?accountid=".$accountid."&cninvoice=1&vehicleid=".$vehicleid."&typeid=".$origtype."\"><input type=\"button\" class=\"quotebutton\" value=\"Create a New ".$origtypename." \"></a></td></tr>\n";
?></tbody></table>
