...
$title = 'Insert';
$linkpage = 'inserttransactions.php';
$changecustomer = '0';
...
?>
</select></div><input type="hidden" name="form" value="1"></td></form><td><form name="update" action="accounthistory.php" method="POST"><input type="hidden" name="accountid" value="<?php echo $accountid; ?>">
<?php
if($vehicleid > '0')
{
...
?>
Include Voided Transactions<input type="checkbox" name="voided"></td><td> Show Only:</td><td> <div class="styled-select black rounded"><select name="invoicetype" >
	<?php
	if($selectedtype > '0')
	{
...
?>
<option value="1">Invoices</option><option value="4">Quotes</option><option value="6">Payments</option><option value="10">Financial Transactions</option><option value="11">Unpaid Invoices</option></select></div></td><td>Start Date: <input type="date" name="startdate" value="<?php echo $startdate; ?>"></td><td>End Date: <input type="date" name="enddate" value="<?php echo $enddate; ?>"></td><td><input type="hidden" name="submit" value="1"><input type="submit" class="smallbutton" value="Update Search"></td></form></tr>
<tr href="newtransaction"><td colspan="4"><br /><input type="button" class="quotebutton" value="Insert New Transaction"></td></tr></table>
</div>
        <div id="content">
...
if($vehicleid > '0')
{
$sql1 = 'SELECT `id`,`invoicedate`,`type`,`total`,`voiddate`,`vehicleid`,`mileagein`,`taxgroup` FROM `invoice` WHERE `vehicleid` = :vehicleid '.$void.'AND `invoicedate` > :startdate AND `invoicedate` '.$typeselect.'< :enddate '.$invtype.'ORDER BY `invoicedate` DESC';
}else{
$sql1 = 'SELECT `id`,`invoicedate`,`type`,`total`,`voiddate`,`vehicleid`,`mileagein`,`taxgroup` FROM `invoice` WHERE `accountid` = :accountid '.$void.'AND `invoicedate` > :startdate AND `invoicedate` '.$typeselect.'< :enddate '.$invtype.'ORDER BY `invoicedate` DESC';
}
$sth1 = $pdocxn->prepare($sql1);
if($vehicleid > '0')
{
...
${"ip".$tri} .= "</table></div></div>";
	echo "<tr href=\"$tri\"><td>$displaydate</td><td><b>$voidedbrand $typename</b></td><td>$displayinvtotal</td><td>$abvvehicle</td><td>$mileage</td></tr>\n";
$tri ++;
}
else {
${"ip".$tri} .= "</table></div></div>";
