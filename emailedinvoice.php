...
$sth3 = $pdocxn->prepare('SELECT * FROM `customeremail` WHERE `invoiceid` = :inv AND `salt` = :salt');
$sth3->bindParam(':inv',$invoiceid);
$sth3->bindParam('salt',$salt);
$sth3->execute();
if ($sth3->rowCount() > 0)
{
...
$sth5 = $pdocxn->prepare('SELECT * FROM `accountbalance` WHERE `accountid` = :acct');
...
$payrow = '1';
$sql11 = "SELECT `id`,`totallineamount`,`lineitem_typeid`,`comment` FROM `line_items` WHERE `invoiceid` = :paymentid LIMIT 1";
...
{
echo "Sorry, there was an issue with retrieving this invoice";
}
?>